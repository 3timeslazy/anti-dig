...
func TestEndToEndSuccess(t *testing.T) {
	...
	t.Run("non-error return arguments from invoke are ignored", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			`missing dependencies for function "github.com/3timeslazy/dig_test".TestEndToEndSuccess.func\S+`,
...
func TestGroups(t *testing.T) {
	...
	t.Run("failure to build a grouped value fails everything", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			`could not build arguments for function "github.com/3timeslazy/dig_test".TestGroups`,
			`could not build value group string\[group="x"\]:`,
			`received non-nil error from function "github.com/3timeslazy/dig_test".TestGroups\S+`,
			`dig_test.go:\d+`, // file:line
			"great sadness",
		)
...
func TestRecoverFromPanic(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			invoke: func(i int) {},
			wantErr: []string{
				`could not build arguments for function "github.com/3timeslazy/dig_test".TestRecoverFromPanic.\S+`,
				`failed to build int:`,
				`panic: "terrible sadness" in func: "github.com/3timeslazy/dig_test".TestRecoverFromPanic.\S+`,
			},
		},
		{
			...
			wantErr: []string{
				...
				`panic: "great sadness" in func: "github.com/3timeslazy/dig_test".TestRecoverFromPanic.\S+`,
			},
		},
		{
			...
			wantErr: []string{
				`panic: "terrible woe" in func: "github.com/3timeslazy/dig_test".TestRecoverFromPanic.\S+`,
			},
		},
	}
	...
	t.Run("constructor consumes a dig.Out", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.desc, func(t *testing.T) {
				...
				dig.AssertErrorMatches(t, err,
					`cannot provide function "github.com/3timeslazy/dig_test".TestProvideConstructorErrors\S+`,
					`dig_test.go:\d+`, // file:line
					`bad argument 1:`,
...
func TestProvideRespectsConstructorErrors(t *testing.T) {
	...
	t.Run("constructor fails", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			`could not build arguments for function "github.com/3timeslazy/dig_test".TestProvideRespectsConstructorErrors\S+`,
			`dig_test.go:\d+`, // file:line
			`failed to build \*bytes.Buffer:`,
			`received non-nil error from function "github.com/3timeslazy/dig_test".TestProvideRespectsConstructorErrors\S+`,
			`dig_test.go:\d+`, // file:line
			`oh no`)
...
func TestProvideLocation(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), `"github.com/3timeslazy/dig_test".testStruct.TestMethod`)
...
func TestCantProvideParameterObjects(t *testing.T) {
	...
	t.Run("constructor", func(t *testing.T) {
		...
		require.Error(t, err, "provide should fail")
		dig.AssertErrorMatches(t, err,
			`cannot provide function "github.com/3timeslazy/dig_test".TestCantProvideParameterObjects\S+`,
			`dig_test.go:\d+`, // file:line
			"bad result 1:",
...
func testProvideCycleFails(t *testing.T, dryRun bool) {
	...
	t.Run("parameters only", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			`cannot provide function "github.com/3timeslazy/dig_test".testProvideCycleFails.\S+`,
			`dig_test.go:\d+`, // file:line
			`this function introduces a cycle:`,
			`func\(\*dig_test.C\) \*dig_test.A provided by "github.com/3timeslazy/dig_test".testProvideCycleFails\S+ \(\S+\)`,
			`depends on func\(\*dig_test.B\) \*dig_test.C provided by "github.com/3timeslazy/dig_test".testProvideCycleFails.\S+ \(\S+\)`,
			`depends on func\(\*dig_test.A\) \*dig_test.B provided by "github.com/3timeslazy/dig_test".testProvideCycleFails.\S+ \(\S+\)`,
			`depends on func\(\*dig_test.C\) \*dig_test.A provided by "github.com/3timeslazy/dig_test".testProvideCycleFails.\S+ \(\S+\)`,
		)
	...
	t.Run("dig.In based cycle", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			...
			`dig_test.go:\d+`, // file:line
			`this function introduces a cycle:`,
			`func\(dig_test.AParams\) dig_test.A provided by "github.com/3timeslazy/dig_test".testProvideCycleFails\S+ \(\S+\)`,
			`depends on func\(dig_test.CParams\) dig_test.C provided by "github.com/3timeslazy/dig_test".testProvideCycleFails.\S+ \(\S+\)`,
			`depends on func\(dig_test.BParams\) dig_test.B provided by "github.com/3timeslazy/dig_test".testProvideCycleFails.\S+ \(\S+\)`,
			`depends on func\(dig_test.AParams\) dig_test.A provided by "github.com/3timeslazy/dig_test".testProvideCycleFails.\S+ \(\S+\)`,
		)
		assert.Error(t, c.Invoke(func(c C) {}), "expected invoking a function that uses a type that failed to provide to fail.")
	})
	...
	t.Run("group based cycle", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			...
			`dig_test.go:\d+`, // file:line
			`this function introduces a cycle:`,
			`func\(\*dig_test.D\) dig_test.outB provided by "github.com/3timeslazy/dig_test".testProvideCycleFails\S+ \(\S+\)`,
			`depends on func\(dig_test.inD\) \*dig_test.D provided by "github.com/3timeslazy/dig_test".testProvideCycleFails.\S+ \(\S+\)`,
			`depends on func\(dig_test.inC\) dig_test.outC provided by "github.com/3timeslazy/dig_test".testProvideCycleFails.\S+ \(\S+\)`,
			`depends on func\(\*dig_test.D\) dig_test.outB provided by "github.com/3timeslazy/dig_test".testProvideCycleFails.\S+ \(\S+\)`,
		)
	})
	...
	t.Run("DeferAcyclicVerification eventually catches cycle with self-cycle", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			`cycle detected in dependency graph:`,
			`func\(\*dig_test.C\) \*dig_test.C provided by "github.com/3timeslazy/dig_test".testProvideCycleFails.\S+ \(\S+\)`,
			`depends on func\(\*dig_test.C\) \*dig_test.C provided by "github.com/3timeslazy/dig_test".testProvideCycleFails.\S+ \(\S+\)`,
		)
	})
}
...
func testProvideFailures(t *testing.T, dryRun bool) {
	t.Run("out returning multiple instances of the same type", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			`cannot provide function "github.com/3timeslazy/dig_test".testProvideFailures\S+`,
			`dig_test.go:\d+`, // file:line
	...
	t.Run("provide multiple instances with the same name", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			...
			`already provided by "github.com/3timeslazy/dig_test".testProvideFailures\S+`,
		)
	})
	...
	t.Run("error should refer to location given by LocationForPC ProvideOption", func(t *testing.T) {
		...
		require.Error(t, err, "provide must return error")
		dig.AssertErrorMatches(t, err,
			`cannot provide function "github.com/3timeslazy/dig_test".testProvideFailures.func\d+.1`,
		)
	})
}
...
func testInvokeFailures(t *testing.T, dryRun bool) {
	...
	t.Run("unmet dependency", func(t *testing.T) {
		...
		require.Error(t, err, "expected failure")
		dig.AssertErrorMatches(t, err,
			`missing dependencies for function "github.com/3timeslazy/dig_test".testInvokeFailures\S+`,
			`dig_test.go:\d+`,
			`missing type:`,
			`\*bytes.Buffer`,
		)
	...
	t.Run("unmet constructor dependency", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			`could not build arguments for function "github.com/3timeslazy/dig_test".testInvokeFailures\S+`,
			`dig_test.go:\d+`, // file:line
	...
	t.Run("constructor invalid optional tag", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			`cannot provide function "github.com/3timeslazy/dig_test".testInvokeFailures\S+`,
			`dig_test.go:\d+`, // file:line
			"bad argument 1:",
	...
	t.Run("optional dep with failed transitive dep", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			...
			`received non-nil error from function "github.com/3timeslazy/dig_test".testInvokeFailures.\S+`,
			`dig_test.go:\d+`, // file:line
			`failed`,
		)
	...
	t.Run("direct dependency error", func(t *testing.T) {
		...
		require.Error(t, err, "expected Invoke error")
		dig.AssertErrorMatches(t, err,
			`received non-nil error from function "github.com/3timeslazy/dig_test".testInvokeFailures.func\S+`,
			`dig_test.go:\d+`, // file:line
			"great sadness",
		)
	...
	t.Run("direct parameter object error", func(t *testing.T) {
		...
		require.Error(t, err, "expected Invoke error")
		dig.AssertErrorMatches(t, err,
			`could not build arguments for function "github.com/3timeslazy/dig_test".testInvokeFailures.func\S+`,
			"failed to build dig_test.A:",
			`received non-nil error from function "github.com/3timeslazy/dig_test".testInvokeFailures.func\S+`,
			`dig_test.go:\d+`, // file:line
			"great sadness",
...
func TestEndToEndSuccessWithAliases(t *testing.T) {
	...
	t.Run("duplicate provide", func(t *testing.T) {
		...
		dig.AssertErrorMatches(t, err,
			`cannot provide function "github.com/3timeslazy/dig_test".TestEndToEndSuccessWithAliases\S+`,
			`dig_test.go:\d+`, // file:line
			`cannot provide dig_test.A from \[0\]:`,
			`already provided by "github.com/3timeslazy/dig_test".TestEndToEndSuccessWithAliases\S+`,
		)
	})
	...
	t.Run("duplicate provide with LocationForPC", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), `cannot provide function "github.com/3timeslazy/dig_test".testStruct.TestMethod`)
		require.Contains(t, err.Error(), `already provided by "github.com/3timeslazy/dig_test".testStruct.TestMethod`)
	})
